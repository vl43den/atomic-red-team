...
This test uses the Python 3 version of MimiPenguin instead of the shell script which requires Python 2.
...
| MimiPenguin_Location | Path of MimiPenguin Python script | path | /tmp/mimipenguin/mimipenguin_2.0-release/mimipenguin.py|
...
```bash
sudo python3 #{MimiPenguin_Location} > #{output_file}
cat #{output_file}
```
